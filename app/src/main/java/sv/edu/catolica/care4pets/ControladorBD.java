...
public class ControladorBD extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE Mascotas(ID_pet integer primary key,Nombre text, Apellido text, Raza text,Sexo text,Especie text, Color text,FechaNaci date,Esterilizacion text,FechaEsterilizacion date)");
        db.execSQL("CREATE TABLE Evento(ID_Evento integer primary key,Nombre text, Fecha date,Hora time,Mascota text,TipoEvento text,Descripcion text)");
        db.execSQL("CREATE TABLE Medicamentos(ID_Medicamento integer primary key,Nombre text,dosis integer,hora_ini time,notas text)");
